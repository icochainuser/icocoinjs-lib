...
describe('icocoinjs-lib (transactions)', function () {
  it('can create a 1-to-1 Transaction', function () {
    var alice = icocoin.ECPair.fromWIF('L1uyy5qTuGrVXrmrsvHWHgVzW9kKdrp27wBC7Vs6nZDTF2BRUVwy')
    ...
    // prepare for broadcast to the Icocoin network, see "can broadcast a Transaction" below
  ...
  it('can create a 2-to-2 Transaction', function () {
    var alice = icocoin.ECPair.fromWIF('L1Knwj9W3qK3qMKdTvmg3VfzUs3ij2LETTFhxza9LfD5dngnoLG1')
    var bob = icocoin.ECPair.fromWIF('KwcN2pT3wnRAurhy7qMczzbkpY5nXMW2ubh696UBc1bcwctTx26z')
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', function (done) {
    ...
    var alice1 = icocoin.ECPair.makeRandom({ network: testnet })
    var alice2 = icocoin.ECPair.makeRandom({ network: testnet })
    var aliceChange = icocoin.ECPair.makeRandom({ rng: rng, network: testnet })
    ...
    testnetUtils.faucetMany([
      ...
      var txb = new icocoin.TransactionBuilder(testnet)
      ...
      // build and broadcast to the Icocoin Testnet network
      dhttp({
        method: 'POST',
...
//          url: 'http://tbtc.blockr.io/api/v1/tx/push',
      ...
      // to build and broadcast to the actual Icocoin network, see https://github.com/icocoinjs/icocoinjs-lib/issues/839
    })
  })
  ...
  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', function (done) {
    ...
    var keyPair = icocoin.ECPair.makeRandom({ network: testnet })
    ...
    testnetUtils.faucet(address, 5e4, function (err, unspent) {
      ...
      var txb = new icocoin.TransactionBuilder(testnet)
      var data = Buffer.from('icocoinjs-lib', 'utf8')
      var dataScript = icocoin.script.nullData.output.encode(data)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a 2-of-4 P2SH(multisig) input', function (done) {
    ...
    ].map(function (wif) { return icocoin.ECPair.fromWIF(wif, testnet) })
    var pubKeys = keyPairs.map(function (x) { return x.getPublicKeyBuffer() })

    var redeemScript = icocoin.script.multisig.output.encode(2, pubKeys)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a SegWit P2SH(P2WPKH) input', function (done) {
    ...
    var keyPair = icocoin.ECPair.fromWIF('cMahea7zqjxrtgAbB7LSGbcQUr1uX1ojuat9jZodMN87JcbXMTcA', testnet)
    var pubKey = keyPair.getPublicKeyBuffer()
    var pubKeyHash = icocoin.crypto.hash160(pubKey)

    var redeemScript = icocoin.script.witnessPubKeyHash.output.encode(pubKeyHash)
    var redeemScriptHash = icocoin.crypto.hash160(redeemScript)

    var scriptPubKey = icocoin.script.scriptHash.output.encode(redeemScriptHash)
    var address = icocoin.address.fromOutputScript(scriptPubKey, testnet)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a SegWit 3-of-4 P2SH(P2WSH(multisig)) input', function (done) {
    ...
    var witnessScript = icocoin.script.multisig.output.encode(3, pubKeys)
  ...
  it('can verify Transaction signatures', function () {
    ...
    ].map(function (q) { return icocoin.ECPair.fromPublicKeyBuffer(Buffer.from(q, 'hex')) })

    var tx = icocoin.Transaction.fromHex(txHex)

    tx.ins.forEach(function (input, i) {
      var keyPair = keyPairs[i]
      var prevOutScript = icocoin.address.toOutputScript(keyPair.getAddress())
      var scriptSig = icocoin.script.pubKeyHash.input.decode(input.script)
      var ss = icocoin.ECSignature.parseScriptSignature(scriptSig.signature)
