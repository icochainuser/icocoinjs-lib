...
function stealthSend (e, Q) {
  ...
  var c = bigi.fromBuffer(icocoin.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new icocoin.ECPair(null, Q.add(cG))
...
function stealthReceive (d, eG) {
  ...
  var c = bigi.fromBuffer(icocoin.crypto.sha256(eQ.getEncoded()))
  var v = new icocoin.ECPair(d.add(c).mod(n))
...
function stealthRecoverLeaked (v, e, Q) {
  var eQ = Q.multiply(e) // shared secret
  var c = bigi.fromBuffer(icocoin.crypto.sha256(eQ.getEncoded()))
  var d = new icocoin.ECPair(v.subtract(c).mod(n))
...
function stealthDualSend (e, R, Q) {
  ...
  var c = bigi.fromBuffer(icocoin.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new icocoin.ECPair(null, R.add(cG))
...
function stealthDualReceive (d, r, eG) {
  var eQ = eG.multiply(d) // shared secret
  var c = bigi.fromBuffer(icocoin.crypto.sha256(eQ.getEncoded()))
  var v = new icocoin.ECPair(r.add(c).mod(n))
...
describe('icocoinjs-lib (crypto)', function () {
  it('can generate a single-key stealth address', function () {
    ...
    var recipient = icocoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var nonce = icocoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a single-key stealth address (randomly)', function () {
    var recipient = icocoin.ECPair.makeRandom() // private to recipient
    var nonce = icocoin.ECPair.makeRandom() // private to sender
  ...
  it('can generate a dual-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    var recipient = icocoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var scan = icocoin.ECPair.fromWIF('L5DkCk3xLLoGKncqKsWQTdaPSR4V8gzc14WVghysQGkdryRudjBM') // private to scanner/recipient
    var nonce = icocoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a dual-key stealth address (randomly)', function () {
    var recipient = icocoin.ECPair.makeRandom() // private to recipient
    var scan = icocoin.ECPair.makeRandom() // private to scanner/recipient
    var nonce = icocoin.ECPair.makeRandom() // private to sender
