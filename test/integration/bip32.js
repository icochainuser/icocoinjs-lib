...
describe('icocoinjs-lib (BIP32)', function () {
  it('can import a BIP32 testnet xpriv and export to WIF', function () {
    ...
    var node = icocoin.HDNode.fromBase58(xpriv, icocoin.networks.testnet)
  ...
  it('can export a BIP32 xpriv, then import it', function () {
    ...
    var node = icocoin.HDNode.fromSeedBuffer(seed)
    var string = node.toBase58()
    var restored = icocoin.HDNode.fromBase58(string)
  ...
  it('can create a BIP32, icocoin, account 0, external address', function () {
    var path = "m/0'/0/0"
    var root = icocoin.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, icocoin, account 0, external address', function () {
    var root = icocoin.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP49, icocoin testnet, account 0, external address', function () {
    var mnemonic = 'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about'
    var seed = bip39.mnemonicToSeed(mnemonic)
    var root = icocoin.HDNode.fromSeedBuffer(seed)

    var path = "m/49'/1'/0'/0/0"
    var child = root.derivePath(path)

    var keyhash = icocoin.crypto.hash160(child.getPublicKeyBuffer())
    var scriptSig = icocoin.script.witnessPubKeyHash.output.encode(keyhash)
    var addressBytes = icocoin.crypto.hash160(scriptSig)
    var outputScript = icocoin.script.scriptHash.output.encode(addressBytes)
    var address = icocoin.address.fromOutputScript(outputScript, icocoin.networks.testnet)
